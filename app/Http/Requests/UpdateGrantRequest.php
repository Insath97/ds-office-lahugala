...
class UpdateGrantRequest extends FormRequest
{
    ...
    {
        $grantId = $this->route('grant');

        return [
            'client_id' => 'required|exists:clients,id',
            'gn_division_id' => 'required|exists:g_n_divisions,id',
            'code' => 'required|string|max:255|unique:grants,code,' . $grantId,
            'land_registry_no' => 'required|string|max:255|unique:grants,land_registry_no,' . $grantId,
